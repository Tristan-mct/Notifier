...
# =====================================================================================================================
    ...
    def progress(self, str_name, int_total, str_channel=None, str_user=None):
        """Fonction d'envoie d'un message à un utilisateur.
        ...
        if self.internet_on():
            ...
            self.dict_sbars[str_name] = self.slack_progress.new(total=int_total)
            self.dict_sbars[str_name].log(str_name)
        else:
            raise SlackbotException('Erreur de connexion.')
    ...
    def progress_update(self, str_name, int_value):
        ...
        """
        if self.internet_on():
            self.dict_sbars[str_name].pos += (int_value*100/self.dict_sbars[str_name].total)
        else:
            raise SlackbotException('Erreur de connexion.')
    ...
    def progress_log(self, str_name, str_log, bl_stack_log=True):
        ...
        if self.internet_on():
            ...
            self.dict_sbars[str_name].log("{} - {}".format(str_name, str_log))

            # Si on le décide, on supprime les messages loggés avant.
            if not bl_stack_log:
                self.dict_sbars[str_name]._msg_log = [self.dict_sbars[str_name]._msg_log[-1]]
                self.dict_sbars[str_name]._update()
        else:
            raise SlackbotException('Erreur de connexion.')
    ...
    def progress_delete(self, str_name):
        ...
        if self.internet_on():
            ...
            str_ts = self.dict_sbars[str_name].msg_ts
            data = '{"channel":"' + self.dict_sbars[str_name].channel_id + '", "ts":"' + str_ts + '"}'
